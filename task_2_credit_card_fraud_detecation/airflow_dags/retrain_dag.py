...
# 🐍 Direct python path (instead of activating env)
PYTHON_BIN = "/mnt/c/Users/samsu/Desktop/code/fraud_detection/ml_env/bin/python"

# 📁 Scripts folder (you moved it correctly)
...
with DAG(
    ...
    retrain_task = BashOperator(
        ...
        set -e
        cd {PROJECT_DIR}
        {PYTHON_BIN} {SCRIPTS_DIR}/retrain.py > /tmp/retrain_output.json
        """,
    )
    ...
    evaluate_task = BashOperator(
        ...
        {PYTHON_BIN} {SCRIPTS_DIR}/evaluate.py $RUN_ID > /tmp/eval_output.json
        """,
    )
